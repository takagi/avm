...
(subtest "check-free-variable-let"
  ...
  (is-error (check-free-variable-let '(let ((x 1)) x y) nil nil)
            simple-error
            "Base case - free variable in body forms."))
...
(subtest "check-free-variable-flet"
  ...
  (is-error (check-free-variable-flet '(flet ((foo (x) x y)) 1) nil nil)
            simple-error
...
(subtest "check-free-variable-labels"
  ...
  (is-error (check-free-variable-labels '(labels ((foo () x)) 1) nil nil)
            simple-error
            "Base case - free variable in binding.")
